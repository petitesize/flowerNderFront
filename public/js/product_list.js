...
const categoryList = async () => {
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        if (categories[i] === 'ALL') {
            ...
            categoryLink = `<a href="/main/${categories[i].toLowerCase()}.html?category=${categories[i]}">${
                categories[i]
            }</a>`;
        }
    ...
    for (let i = 0; i < realData.length; i++) {
        ...
        if (realData[i].category.toLowerCase() === category.toLowerCase()) {
            ...
            <li class="list-wrap">
                <div class="list-top">
                    <a href="/main/detail.html?id=${realData[i]._id}?category=${realData[i].category}">
                        <img src=${realData[i].main_image.url} alt="제품사진1" class="productImage">
                    </a>
                </div>
                <div class="list-bottom">
                    <a href="/main/detail.html?id=${realData[i]._id}?category=${realData[i].category}" class="list-title">${realData[i].title}</a>
