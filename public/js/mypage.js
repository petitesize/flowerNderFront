...
newPasswordConfirm.addEventListener("keyup", (e) => {
  if (e.keyCode === 13) setUserInfo(e);
});
  ...
  if (newPasswordVal) {
    ...
    if (!newPasswordPattern.test(newPasswordVal)) {
      ...
      if (!newPasswordBox.classList.contains("on")) {
        ...
        newPasswordBox.insertAdjacentHTML(
          "beforeend",
    ...
    if (newPasswordVal && !newPasswordConfirmVal) {
      ...
      if (!newPasswordConfirmBox.classList.contains("on")) {
        ...
        newPasswordConfirmBox.insertAdjacentHTML(
          "beforeend",
  ...
  if (!addressDetailVal) {
    ...
    if (!addressDetailBox.classList.contains("on")) {
      ...
      addressDetailBox.insertAdjacentHTML(
        "beforeend",
        '<div class="check-font"><p>상세주소를 입력하세요.</p></div'
      );
    }
    return false;
  }
